...
module.exports = class PgPersistence {
  ...
  async createTodo(todoListId, title) {
    const CREATE_TODO =
      "INSERT INTO todos" +
      "  (title, todolist_id, username, done)" +
      "  VALUES ($1, $2, $3, 'no')";
  ...
  hasUndoneTodos(todoList) {
    return todoList.todos.some((todo) => todo.done !== "yes");
  }
  ...
  isDoneTodoList(todoList) {
    return (
      ...
      todoList.todos.every((todo) => todo.done === "yes")
    );
  }
  ...
  async setTodoStatus(newStatus, todoListId, todoId) {
    const SET_STATUS =
